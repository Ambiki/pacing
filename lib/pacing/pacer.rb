...
module Pacing
  class Pacer
    ...
    def calculate
      ...
      puts expected_visits(start_date: parse_date(service[:start_date]), end_date: parse_date(service[:end_date]), frequency: service[:frequency], frequency: service[:interval]), "e dey work?"
    ...
    def expected_visits(start_date:, end_date:, frequency:, interval:)
      visitable_days = business_days(start_date, end_date).count
    end
